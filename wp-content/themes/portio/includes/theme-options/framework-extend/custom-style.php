...
if ( ! function_exists( 'portio_dynamic_styles' ) ) {
  function portio_dynamic_styles() {

    // Typography
    $portio_get_typography  = portio_get_typography();
    ...
    // Logo
    $portio_logo_top     = cs_get_option( 'portio_logo_top' );
    $portio_logo_bottom  = cs_get_option( 'portio_logo_bottom' );
...
$layout_boxed = ( $bg_pattern === 'custom-pattern' ) ? 'background-image: url('. esc_url($custom_bg_pattern) .');' : 'background-image: url('. esc_url(PORTIO_IMAGES) . '/patterns/'. $bg_pattern .'.png);';
?>
  .layout-boxed {
    <?php echo   wp_kses_data( $layout_boxed ); ?>
  }
...
if ($portio_meta) {
  $transparent_header =  isset( $portio_meta['transparency_header'] ) ? $portio_meta['transparency_header'] : '' ;
  $transparent_menu_color = isset( $portio_meta['transparent_menu_color'] ) ? $portio_meta['transparent_menu_color'] : '' ;
  $transparent_menu_hover = isset( $portio_meta['transparent_menu_hover_color'] ) ? $portio_meta['transparent_menu_hover_color'] : '' ;
} else {
  $transparent_header = '';
...
<?php
  ...
  echo   wp_kses_data( $portio_get_typography );
  $output = ob_get_clean();
  return $output;
 ...
 * Custom Font Family
 */
if ( ! function_exists( 'portio_custom_font_load' ) ) {
  function portio_custom_font_load() {
...
/* Custom Styles */
if( ! function_exists( 'portio_custom_css' ) ) {
  function portio_custom_css() {
    wp_enqueue_style('portio-default-style', get_template_directory_uri() . '/style.css');
    $output  = portio_custom_font_load();
    $output .= portio_dynamic_styles();

    if( function_exists( 'portio_compress_css_lines' ) ) {
      $custom_css = portio_compress_css_lines( $output );
       wp_add_inline_style( 'portio-default-style', $custom_css );
    }
    portio_typography_fonts();
  }
  add_action( 'wp_enqueue_scripts', 'portio_custom_css' );
}

/* Custom JS */
if( ! function_exists( 'portio_custom_js' ) ) {
  function portio_custom_js() {
    $output = cs_get_option( 'theme_custom_js' );
    if ( $output ) {
      wp_add_inline_script( 'jquery-migrate', $output );
    }
  }
  add_action( 'wp_enqueue_scripts', 'portio_custom_js' );
}