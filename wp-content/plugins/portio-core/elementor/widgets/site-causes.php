<?php
/*
 * Elementor Portio Causes Widget
...
class Site_Causes extends Widget_Base{
	...
	*/
	public function get_title(){
		return esc_html__( 'Causes', 'portio-core' );
	}
	...
	*/
	...
	/**
	 * Retrieve the list of scripts the Portio Causes widget depended on.
	...
	*/
	...
	/**
	 * Register Portio Causes widget controls.
    ...
    if ( $posts ) {
      ...
      $PostID[ __( 'No ID\'s found', 'portio' ) ] = 0;
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Causes Options', 'portio-core' ),
			]
		);
		$this->add_control(
			'causes_style',
			[
				'label' => esc_html__('Causes Style', 'portio-core'),
				...
				'default' => 'style-one',
				'description' => esc_html__('Select your causes style.', 'portio-core'),
			]
		);
		$this->add_control(
			'shape_image',
			[
				'label' => esc_html__( 'Shape Image', 'portio-core' ),
		...
		$this->add_control(
			'shape_image2',
			[
				'label' => esc_html__( 'Shape Image 2', 'portio-core' ),
				'type' => Controls_Manager::MEDIA,
				'condition' => [
					'causes_style' => array('style-two'),
				],
		...
		$this->add_control(
			'causes_limit',
			[
				'label' => esc_html__( 'Causes Limit', 'portio-core' ),
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .portio-causes .wpo-campaign-item .wpo-campaign-content small',
			]
		);
		$this->add_control(
			'subtitle_color',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .portio-causes .wpo-campaign-item .wpo-campaign-content small' => 'color: {{VALUE}};',
				],
			]
		);	
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .portio-causes .wpo-campaign-item .wpo-campaign-content small' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->end_controls_section();// end: Section
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Prograss', 'portio-core' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		$this->add_control(
	...
	*/
    ...
    if ($causes_show_id) {
      ...
      'post__in' => $causes_show_id,
		...
		$portio_causes = new \WP_Query( $args );
        ...
        <div class="<?php echo esc_attr( $container_class ); ?>">
          <div class="row g-0 align-items-center">
    				<?php 
    				$unique_id = 0;
						if ( $portio_causes->have_posts()) : while ( $portio_causes->have_posts()) : $portio_causes->the_post();
							$unique_id ++;
                  ...
                  <div class="content">
                      ...
                      <div class="progress-item">
                          ...
                          <div class="progres-label">
                              <div class="label-on">
                                  <p>
                                  	<?php echo esc_html__( 'Raised:','portio-core' ); ?>
                                  	<span>
                              ...
                              <div class="label-two">
                                  <p><?php echo esc_html__( 'Goal:','portio-core' ); ?><span><?php echo apply_filters( 'give_goal_amount_target_output', give_currency_filter( $goal ) ); ?></span></p>
                              </div>
                          </div>
                      </div>
                  </div>
